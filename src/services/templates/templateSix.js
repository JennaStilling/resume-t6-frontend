export const loadTemplateSix = (user, dropdownSections) => {
  const styles = `
    .content { 
  ...
  if (selectedSkills.length) {
    ...
    selectedSkills.forEach(item => {
      content += `<div style="background-color: black; color: white; padding: 5px 10px; border-radius: 5px; margin: 5px;">${item.name}</div>`;
    });
    content += '</div>';
    content += `<div style="height: 5px;"></div>`;
  }
